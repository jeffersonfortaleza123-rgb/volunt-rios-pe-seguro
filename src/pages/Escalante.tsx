      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <h1 className="text-2xl font-bold text-military-dark">Relatório de Voluntários</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <Users className="h-8 w-8 text-military-green" />
                <div>
                  <p className="text-2xl font-bold text-military-dark">{voluntarios.length}</p>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <Calendar className="h-8 w-8 text-military-green" />
                <div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-military-dark">Relatório por Dia</CardTitle>
            <CardDescription>
              Voluntários disponíveis para cada dia do mês atual
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {getDaysInMonth().map(day => (
                <div key={day} className="flex items-start gap-4 p-3 border rounded-lg hover:bg-military-light/30 transition-colors">
                    ...
                    {relatorioDias[day] && relatorioDias[day].length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {relatorioDias[day].map((nome, index) => (
                          <span key={index} className="text-military-dark">
                            {nome}
                  ...
                  {relatorioDias[day] && (
                    <Badge variant="secondary" className="ml-auto bg-military-light text-military-dark">
        ...
        {voluntarios.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-military-dark">Lista Detalhada de Voluntários</CardTitle>
              <CardDescription>
                Informações completas dos militares que se voluntariaram
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {voluntarios.map((voluntario, index) => (
                  <div key={index} className="border rounded-lg p-4 bg-card">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="font-semibold text-military-dark">{voluntario.nome}</h4>
                        <p className="text-sm text-muted-foreground">Matrícula: {voluntario.matricula}</p>
                      </div>
                      <Badge variant="secondary" className="bg-military-light text-military-dark">
        ...
        {voluntarios.length === 0 && (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <h3 className="text-lg font-semibold text-military-dark mb-2">Nenhum voluntário registrado</h3>
              <p className="text-muted-foreground">
                Aguardando militares se voluntariarem para as escalas do mês.
              </p>
            </CardContent>
          </Card>
        )}
